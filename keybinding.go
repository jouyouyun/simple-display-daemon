...
func (m *Manager) handleKeyPressEvent(ev xproto.KeyPressEvent) {
	...
	case isAccelEqual(m.xu, accel, "mod4-r"):
		...
		if startddeLuanched {
			go func() {
				runApp("killall dde-session-daemon dde-dock compiz gtk-window-decorator fcitx mousearea dde-desktop dde-launcher deepin-menu dde-dock-applets dde-control-center")
